...
function buildTable(table, sheet) {
	...
	for (let i = 0; i < dryad_y.length; i++) {
		...
		row = table.insertRow();
		row.innerHTML =
			"<td></td>" + dryad_x.map((_, i) => `<td>${i}</td>`).join("");
...
function encrypt_num(sheet, first, second, plaintext) {
	...
	// numeric encryption
	let encrypted = `${fromNato(first)} ${fromNato(second)} `;
...
function parseLine(input) {
	...
	let first = lines[0].trim().toUpperCase();
	let second = lines[1].trim().toUpperCase();
	let text = lines.slice(2).join("\n").trim();
	return [first, second, text];
}

function decryptNumeric(sheet, ct){
	return ct.map((line) => {
		let [first, second, text] = parseLine(line);
		let [_a, _b, sl] = obtainSetLetter(sheet, first, second);

		return text.split('').map((c) => {
			let [col, subcol] = authFindIndex(sheet[sl], c);
			return col
		}).join('');
	});
}

function decryptText(sheet, ct){
	return ct.map((line) => {
		let [first, second, text] = parseLine(line);
		let [_a, _b, sl] = obtainSetLetter(sheet, first, second);
		...
		return text.split('').map((c) => {
			let [col, subcol] = authFindIndex(sheet[sl], c);
			return alpha_indexes_rev[[col, subcol]];
		}).join('');
	});
}
